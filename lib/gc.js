...
var debug = require('debug')('docker-worker:garbageCollector');
...
GarbageCollector.prototype = {
  ...
  removeContainers: function* () {
    for (var containerId in this.markedContainers) {
      ...
      if (this.markedContainers[containerId].retries !== 0) {
        ...
        this.log('[alert operator] container removal error',
    ...
    for (var image in this.markedImages) {
      ...
      if (runningImages.indexOf(image) === -1) {
        ...
        try {
          ...
          debug('image removed', {image: imageDetails});
          ...
          this.log('[alert operator] image removal error.',
        ...
        debug('garbage collection warning',
  ...
  sweep: function () {
    ...
    debug('garbage collection started');
    co(function* () {
