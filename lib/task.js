  ...
  */
  async dockerConfig(links) {
    ...
    let procConfig = {
      start: {
        Binds: [],
        Privileged: this.runtime.docker.privileged
      },
      create: {
        Image: config.image,
        Cmd: config.command,
    ...
    if (this.runtime.ci) {
      procConfig.start.Binds.push('/var/run/docker.sock:/run/docker.sock');
      procConfig.start.Binds.push('/usr/bin/docker:/bin/docker');
      let testCreds = [
        `TASKCLUSTER_CLIENT_ID=${this.runtime.test.taskcluster.clientId}`,
        `TASKCLUSTER_ACCESS_TOKEN=${this.runtime.test.taskcluster.accessToken}`,
        `PULSE_USERNAME=${this.runtime.test.pulse.username}`,
        `PULSE_PASSWORD=${this.runtime.test.pulse.password}`
      ];

      procConfig.create.Env = procConfig.create.Env.concat(
          testCreds
      );
    }
