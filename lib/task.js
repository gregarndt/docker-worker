...
export default class Task {
  ...
  */
  async dockerConfig(links) {
    ...
    if (this.task.payload.features && 'privileged' in this.task.payload.features) {
      if (!this.runtime.dockerConfig.allowPrivileged) {
        throw new Error(
          'Cannot run task using docker privileged mode.  Worker ' +
          'must be enabled to allow running of priviledged tasks.'
        );
      }
    }
    ...
    let procConfig = {
      start: {
        Binds: [],
        Privileged: this.runtime.dockerConfig.allowPrivileged
      },
      create: {
        Image: config.image,
        Cmd: config.command,
